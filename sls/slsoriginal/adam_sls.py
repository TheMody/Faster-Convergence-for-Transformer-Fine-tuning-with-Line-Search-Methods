    ...
    def step(self, closure):
        ...
        if self.gv_option in ['scalar']:
            ...
            for i, g in enumerate(grad_current):
                if isinstance(g, torch.Tensor) and isinstance(self.state['gv'][i], torch.Tensor):
                    if g.device != self.state['gv'][i].device:
                        self.state['gv'][i] = self.state['gv'][i].to(g.device)
                if isinstance(g, torch.Tensor) and isinstance(self.state['mv'][i], torch.Tensor):
                    if g.device != self.state['mv'][i].device:
                        self.state['mv'][i] = self.state['mv'][i].to(g.device)
